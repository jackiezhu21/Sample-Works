...
public class Nature implements Drawable {
    ...
    private Class[] animals = {Panda.class, Boar.class, Whale.class, Cow.class};
    private Class[] edibles = {Boar.class, Cow.class};
    private Class[] endangered = {Panda.class, Whale.class};
     ...
     */
    private static <E> E randomSelection(E[] arr) {
        int len = arr.length;
        ...
        for (int i = 0; i < chosenOnes.size(); ++i) {
            if (chosenOnes.get(i) != null) {
                ...
                if (selected instanceof  EndangeredSpecies) {
                    if (selected.isHit(scope)) {
                        num += ((EndangeredSpecies) selected)
                            .getWantedLevel();
                        existingAnimals.remove(selected);
                        chosenOnes.remove(selected);
                    }
    ...
    /**
     * Draw background in HungerGame
     * @param g Graphics representation of background
     */
    public void draw(Graphics g) {
        for (Animal existingAnimal : existingAnimals) {
            existingAnimal.draw(g);
        }
        for (Home home : homes) {
            home.draw(g);
        }
